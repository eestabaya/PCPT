...
def extract_bh_page(source):
    ...
    for product in products:
        ...
        product_specs_block = product_soup.find_all(
            lambda tag: tag.name == 'li' and tag.get('data-selenium') == 'miniProductPageSellingPointsListItem')

        product_specs = []
        for product_spec in product_specs_block:
            product_specs.append(product_spec.contents[0])

        part_num_to_data[product_id] = \
            [product_price, product_name, product_link, product_picture, product_rating, product_specs]
